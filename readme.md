...
* [*OpenCV* 4.2.0](https://github.com/opencv/opencv/archive/4.2.0.zip) with the `viz` module, *QT* support, `pkg-config` support and non-free modules enabled as well as the [contributed modules](https://github.com/opencv/opencv_contrib/archive/4.2.0.zip) `xfeatures2d`, `stitching` and `sfm` included (see [recommended build command](opencv_config.md)),
...
All demonstrations have been tested on a [64-bit *Ubuntu* 18.04.3 minimal](http://archive.ubuntu.com/ubuntu/dists/bionic-updates/main/installer-amd64/current/images/netboot/mini.iso) system with a *Xubuntu minimal installation* and the following installed packages: `build-essential` (for *g++* 7.4.0 and *make*), `qt5-default` (for *QT* 5.9.5), `libvtk7-dev` (for *VTK* 7.1.1 which is required for *OpenCV*'s `viz` module), `libeigen3-dev`, `libgflags-dev`, `libgoogle-glog-dev` (all of which are required for *OpenCV*'s `sfm` module) and `libasound2-dev` (for *ALSA*).
